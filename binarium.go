...
func runBinarium(
	...
	if transactionID != "" {
		go checkTransaction(transactionID, asset, kind, betAgainIfFailed)
	}
	...
	if loginRequired {
		...
		login()
		logger.Infof("%s restart on relogin", loggerPrefix)
		runBinarium(sum, asset, kind, betAgainIfFailed)
	}
}
...
func checkTransaction(
	...
	if !transactionSuccess {
		...
		if betAgainIfFailed {
			...
			runBinarium(failedBetSum, asset, kind, betAgainIfFailed)
		}
	}
